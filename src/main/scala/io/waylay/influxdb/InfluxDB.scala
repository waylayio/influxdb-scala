...
object InfluxDB{
  ...
  object Duration{
    ...
    def days(amount: Int) = Duration(amount, Day)
    def weeks(amount: Int) = Duration(amount, Week)
    def hours(amount: Int) = Duration(amount, Hour)
    def minutes(amount: Int) = Duration(amount, Minute)
    def seconds(amount: Int) = Duration(amount, Second)
    def milliseconds(amount: Int) = Duration(amount, MilliSecond)
    def microseconds(amount: Int) = Duration(amount, MicroSecond)
  }
...
class InfluxDB(
  ...
  //var database: String = "",
  schema: String = "http",
  defaultRetention: String = "INF")(implicit ec: ExecutionContext) {
  ...
  def query(databaseName: String, query: String, chunkSize: Option[Int] = None, epoch: Option[Epoch] = None): Future[Results] = {
    logger.debug(query)
  ...
  //
  def storeAndMakeDbIfNeeded(databaseName: String, points: Seq[IPoint], createDbIfNeeded: Boolean = true, precision: TimeUnit = MILLISECONDS): Future[Unit] = {
    val data = WriteProtocol.write(precision, points.toSeq:_*)
    ...
    req.post(data).flatMap{ response =>
      ...
      response.status match {
        case 404 if createDbIfNeeded =>
          ...
          createDb(databaseName).flatMap(_ =>
            storeAndMakeDbIfNeeded(databaseName, points, createDbIfNeeded = false)
          )
        case 204 => // ok
